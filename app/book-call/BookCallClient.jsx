...
import Success from "../book/[slug]/components/Success";
import Calendar from "../book/[slug]/components/calender";
import { useEventType } from "../book/[slug]/hooks/useEventType";
import { useSlots } from "../book/[slug]/hooks/useSlots";
import { getTodayDateString } from "../book/[slug]/utils/dateUtils";
import { useTheme } from "../context/themeContext";

export default function BookCallClient() {
  const { theme } = useTheme();
  const searchParams = useSearchParams();
  const querySlug = searchParams?.get?.("slug") || "discovery-call";
        ...
        <div className="max-w-full mx-auto">
          <div className="mt-8 mb-8">
            <h1 className={`text-2xl sm:text-3xl font-bold ${theme === "light" ? "text-gray-900" : "text-white"}`}>
              Book a <span className={`${theme === "light" ? "text-orange-500" : "text-orange-500"}`}>Meeting</span>
            </h1>
            <p className={`text-sm ${theme === "light" ? "text-gray-600" : "text-gray-300"} mt-2`}>
              Share this public link to let others book time with you. Tip: append ?slug=your-event-slug to change event type.
            </p>
          </div>
