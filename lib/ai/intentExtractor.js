...
- If question asks for counts/totals/rates (e.g., "how many", "total", "count") → query_type = "aggregate"
  * MUST use a valid metric from SUPPORTED METRICS list
  * Examples: "How many leads?", "Total leads", "Lead count"
...
- If question asks for multiple items/listings (e.g., "show all...", "list...", "get all...", "leads assigned to...", "leads from...") → query_type = "list"
  * CRITICAL: Questions with filters like "assigned_to", "status", "industry_type", etc. are ALWAYS "list" queries, NOT "aggregate"
  * Examples: "Leads assigned to John" → list, "Pending leads" → list, "Healthcare leads" → list
  * Only use "aggregate" when explicitly asking for a COUNT/NUMBER

---------------------------------------
CRITICAL: METRICS vs COLUMNS (MANDATORY)
---------------------------------------
METRICS and COLUMNS are NOT the same thing.

ALLOWED METRICS (ONLY THESE - NEVER INVENT OTHERS):
${ALLOWED_METRICS.join(", ")}

COLUMNS (like assigned_to, status, priority, lead_source, current_stage, industry_type, etc.) 
MUST ONLY be used as FILTERS, NEVER as metrics.

METRIC SELECTION RULES:
- "How many leads ... ?" → metric = "lead_count"
- "Total leads ..." → metric = "lead_count"
- "Count of leads ..." → metric = "lead_count"
- "How many qualified leads ... ?" → metric = "qualified_lead_count"
- "How many prospects ... ?" → metric = "prospect_count"
- "How many meetings scheduled ... ?" → metric = "meeting_scheduled_count"
- "How many meetings conducted ... ?" → metric = "meeting_conducted_count"
- "Pending tasks ..." → metric = "task_pending_count"
- "Overdue tasks ..." → metric = "task_overdue_count"
- "Calls made ..." → metric = "call_count"
- "Followups done ..." → metric = "followup_count"
- "Bookings ..." → metric = "booking_count"
- "Conversion chance / probability ..." → metric = "conversion_probability"

FILTER EXTRACTION RULES:
Any phrase that narrows data MUST be extracted as FILTERS, not metrics:
- "assigned to <user>" → filters: {"assigned_to": "<user>"}
- "status = <value>" → filters: {"status": "<value>"}
- "priority = <value>" → filters: {"priority": "<value>"}
- "stage = <value>" → filters: {"current_stage": "<value>"}
- "source = <value>" → filters: {"lead_source": "<value>"}
- "industry = <value>" → filters: {"industry_type": "<value>"}

CORRECT EXAMPLE:
User: "How many leads are assigned to Sp-01"
→ {
    "intent_type": "crm_query",
    ...
    "table": "leads_table",
    "scope": "global",
    "filters": {"assigned_to": "Sp-01"}
  }

INCORRECT (NEVER DO THIS):
User: "How many leads are assigned to Sp-01"
→ {
    "metric": "assigned_to"  // WRONG! assigned_to is a COLUMN, not a METRIC
  }

FAIL-SAFE RULE:
If a user asks for a count that does not map to an allowed metric, default to:
metric = "lead_count"
and apply all constraints as filters.
...
- "How many leads are assigned to Sp-01?" → {"intent_type": "crm_query", "query_type": "aggregate", "metric": "lead_count", "table": "leads_table", "scope": "global", "filters": {"assigned_to": "Sp-01"}}
- "Total leads with status Pending" → {"intent_type": "crm_query", "query_type": "aggregate", "metric": "lead_count", "table": "leads_table", "scope": "global", "filters": {"status": "Pending"}}
- "How many leads assigned to John?" → {"intent_type": "crm_query", "query_type": "aggregate", "metric": "lead_count", "table": "leads_table", "scope": "global", "filters": {"assigned_to": "John"}}
...
- "Leads assigned to John" → {"intent_type": "crm_query", "query_type": "list", "table": "leads_table", "scope": "global", "filters": {"assigned_to": "John"}}
- "Show leads from healthcare industry" → {"intent_type": "crm_query", "query_type": "list", "table": "leads_table", "scope": "global", "filters": {"industry_type": "healthcare"}}
- "Pending leads" → {"intent_type": "crm_query", "query_type": "list", "table": "leads_table", "scope": "global", "filters": {"status": "Pending"}}

CRITICAL: NEVER use column names as metrics:
- WRONG: {"metric": "assigned_to"} - assigned_to is a COLUMN, use as FILTER
- WRONG: {"metric": "status"} - status is a COLUMN, use as FILTER
- WRONG: {"metric": "priority"} - priority is a COLUMN, use as FILTER
- CORRECT: {"metric": "lead_count", "filters": {"assigned_to": "Sp-01"}}
- "How many leads are assigned to John?" → {"intent_type": "crm_query", "query_type": "aggregate", "metric": "lead_count", "table": "leads_table", "scope": "global", "filters": {"assigned_to": "John"}}

CRITICAL DISTINCTION:
- "Leads assigned to John" → list query (shows the leads)
- "How many leads assigned to John?" → aggregate query (shows count)
- "assigned_to", "status", "industry_type" are FILTERS, NOT METRICS
- Only use "aggregate" query_type when question explicitly asks for a COUNT/NUMBER
