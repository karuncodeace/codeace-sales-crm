...
export function generateTaskTitle(stage, leadName, options = {}) {
  ...
  // Validate stage
  const validStages = ["New", "Contacted", "Demo", "Proposal", "Follow Up", "Won"];
  ...
  if (normalizedStage === "Won") {
    const error = new Error("Cannot create tasks for leads in 'Won' stage");
    console.error("Task title generation error:", error);
    throw error;
  }
  ...
  switch (normalizedStage) {
    ...
    case "Proposal":
      return `Proposal Follow-up – ${displayName}`;
    
    case "Follow Up":
      return `Follow Up Call – ${displayName}`;
