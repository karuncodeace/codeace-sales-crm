...
export async function getCrmUser() {
  try {
    ...
    // Match email with sales_persons table (case-insensitive)
    // Try exact match first, then case-insensitive
    let { data: salesPerson, error: salesPersonError } = await supabase
      .from("sales_persons")
    ...
    if (salesPersonError && salesPersonError.code === 'PGRST116') {
      ...
      if (allSalesPersons) {
        salesPerson = allSalesPersons.find(
          sp => sp.email && sp.email.toLowerCase() === user.email.toLowerCase()
        );
        salesPersonError = salesPerson ? null : { code: 'NOT_FOUND', message: 'No matching sales person' };
      }
    }

    if (salesPersonError || !salesPerson) {
      console.warn("Sales person not found for email:", user.email, "Error:", salesPersonError?.message || salesPersonError);
      return null;
    }
    ...
    console.log("getCrmUser: Found user", { id: salesPerson.id, role: normalizedRole, email: salesPerson.email });
