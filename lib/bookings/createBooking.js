...
import { supabaseServer, supabaseAdmin } from "../supabase/serverClient";
 ...
 * Uses same overlap logic as slot generator: booking overlaps our buffered range
 * when booking.start_time < ourBufferedEnd AND booking.end_time > ourBufferedStart.
 */
async function checkForConflicts(
  ...
  const startIso = bufferedStartUtc.toISOString();
  const endIso = bufferedEndUtc.toISOString();
  ...
  if (conflicts.length > 0) {
    console.warn("Booking conflict check found overlapping bookings:", {
      eventTypeId,
      bufferedRange: { start: startIso, end: endIso },
      conflictCount: conflicts.length,
      conflictIds: conflicts.map((b) => b.id),
    });
  }
...
export async function createBooking(requestBody) {
  ...
  // Use admin client for conflict check so we see the same bookings as slot generation (generateSlots uses admin).
  // This avoids "Selected slot is no longer available" when RLS would otherwise show different rows.
  ...
  if (hasConflict) {
    console.warn("Slot rejected: conflict check found overlapping booking(s) for this time range.");
    throw new Error("Selected slot is no longer available");
  }

  // Generate unique id so we never hit duplicate key (bookings.id has no DB default)
  const bookingId = `AP-${randomUUID()}`;
  ...
  const bookingData = {
    ...
    guest_mailid: requestBody.guest_mailid || null,
  ...
  if (bookingError) {
    if (bookingError.code === "23505") {
      // Log full error so we can see which constraint failed (constraint name may differ from index name)
      console.warn("Slot rejected: unique constraint violation (23505):", {
