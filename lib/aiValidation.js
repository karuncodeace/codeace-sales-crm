...
export function validateIntent(intent) {
  ...
  if (intent.query_type === "aggregate") {
    if (!intent.metric || !ALLOWED_METRICS.includes(intent.metric)) {
      ...
      const commonFilterFields = ["assigned_to", "status", "industry_type", "priority", "lead_source", "current_stage"];
      if (intent.metric && commonFilterFields.includes(intent.metric)) {
        return {
          valid: false,
          error: `"${intent.metric}" is a filter field, not a metric. Use query_type "list" with filters: {"${intent.metric}": "value"}. For counts, use query_type "aggregate" with metric "lead_count" and filters: {"${intent.metric}": "value"}.`
        };
      }
      return {
        valid: false,
